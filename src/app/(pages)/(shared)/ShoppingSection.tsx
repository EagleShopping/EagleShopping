...
const ShoppingSection = (props: Props) => {
  const { selectCategory, SetselectCategory, products, isLoading, error } =
    useShopping();
  ...
  return (
    <Box mb="20px">
      ...
      <CategoryShop
        ...
        SetselectCategory={(category) => SetselectCategory(category)}
      />
      ...
      >
        {Array.isArray(products) &&
          products.map((product) => (
            <CategoryPhotos
              products={product}
              onBuyProduct={(Product) => dispatch(addProducts(Product))}
